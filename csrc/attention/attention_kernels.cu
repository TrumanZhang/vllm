...
template <typename scalar_t, int HEAD_SIZE, int NUM_THREADS,
          int PARTITION_SIZE>
__global__ void sequence_block_reduce_kernel(
    scalar_t* __restrict__ out,            // [num_seqs, num_heads, head_size]
    const float* __restrict__ exp_sums,    // [num_seqs, num_heads,
                                           // max_num_partitions]
    const float* __restrict__ max_logits,  // [num_seqs, num_heads,
...
void sequence_block_reducer_launcher(torch::Tensor& out,
  ...
  const at::cuda::OptionalCUDAGuard device_guard(device_of(tmp_out));
  ...
  switch (head_size) {
    ...
    case 64:
      LAUNCH_SEQUENCE_BLOCK_REDUCER(64);
      break;
    case 80:
      LAUNCH_SEQUENCE_BLOCK_REDUCER(80);
      break;
    case 96:
      LAUNCH_SEQUENCE_BLOCK_REDUCER(96);
      break;
    case 112:
      LAUNCH_SEQUENCE_BLOCK_REDUCER(112);
      break;
    case 128:
      LAUNCH_SEQUENCE_BLOCK_REDUCER(128);
      break;
    case 192:
      LAUNCH_SEQUENCE_BLOCK_REDUCER(192);
      break;
    case 256:
      LAUNCH_SEQUENCE_BLOCK_REDUCER(256);
      break;
    default:
...
#define CALL_V3_LAUNCHER_SPARSITY(T, CACHE_T, BLOCK_SIZE, IS_FP8_KV_CACHE) \
  switch (is_block_sparse) {                                               \
    case true:                                                             \
      CALL_V3_LAUNCHER(T, CACHE_T, BLOCK_SIZE, IS_FP8_KV_CACHE, true);     \
      break;                                                               \
    case false:                                                            \
      CALL_V3_LAUNCHER(T, CACHE_T, BLOCK_SIZE, IS_FP8_KV_CACHE, false);    \
      break;                                                               \
  }
...
#define CALL_V3_LAUNCHER_BLOCK_SIZE(T, CACHE_T, KV_DTYPE)         \
  switch (block_size) {                                           \
    case 8:                                                       \
      CALL_V3_LAUNCHER_SPARSITY(T, CACHE_T, 8, KV_DTYPE);         \
      break;                                                      \
    case 16:                                                      \
      CALL_V3_LAUNCHER_SPARSITY(T, CACHE_T, 16, KV_DTYPE);        \
      break;                                                      \
    case 32:                                                      \
      CALL_V3_LAUNCHER_SPARSITY(T, CACHE_T, 32, KV_DTYPE);        \
      break;                                                      \
    default:                                                      \
      TORCH_CHECK(false, "Unsupported block size: ", block_size); \
      break;                                                      \
